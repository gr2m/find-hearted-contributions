...
async function findHeartedContributions (options) {
  const MyOctokit = options.cache ? Octokit.plugin(require('./lib/cache-plugin')) : Octokit
  ...
  try {
    ...
    for (const repositoryName of state.repositories) {
      ...
      const commentUrls = await getCommitCommentsForRepository(state, repositoryName)
    ...
    const heartedItems = []
    for (const url of [...state.issuesAndPullRequestUrls, ...state.commentsUrls]) {
